...
def calularTiempo(tiempos):
    limite_tiempo = timedelta(hours=7)

    total_entrega = timedelta()
    for tiempo in tiempos:
        h, m, s = map(int, tiempo.split(':'))
        tiempo_entrega = timedelta(hours=h, minutes=m, seconds=s)
        total_entrega += tiempo_entrega

    diferencia = limite_tiempo - total_entrega

    if diferencia < timedelta():
        diferencia = -diferencia
        resultado = f'Han excedido el límite de tiempo: -{str(diferencia)}'
    else:
        resultado = str(diferencia)
    
    return resultado

tiempo = ['00:10:00', '03:45:00', '03:30:00']
print(calularTiempo(tiempo))